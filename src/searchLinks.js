// Search-link builder — the reliable core of the app.
//
// Instead of scraping (fragile, blocked, against ToS), we turn the user's
// criteria into a ready-made search URL for each site. One tap opens that
// site's real, filtered results. This never breaks: where we know a site's
// native filter format we build it directly; otherwise we fall back to a
// Google site-scoped search, which works for any site.
...
/** Free-text terms describing the search, for query-based links. */
function terms(c) {
...
function siteSearch(domain, c) {
  ...
  const q = [`site:${domain}`, terms(c), yearStr, clean(c.city)]
    .filter(Boolean)
    .join(" ")
    .trim() || `site:${domain} רכב יד שנייה`;
